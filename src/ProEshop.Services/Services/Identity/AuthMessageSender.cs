...
using ProEshop.Services.Services.Identity.Smsl;
...
public class AuthMessageSender : ISmsSender
{
    ...
    private async Task<string> GetToken()
    {
        ...
        var result = await _httpClient.SendAsync("https://RestfulSms/api/Token", HttpMethod.Post, content: modelInJson);
        var responseBody = await result.Content.ReadAsStringAsync();
        return JsonConvert.DeserializeObject<SmsToken>(responseBody).TokenKey;
    }

    private async Task<string> GetLineToSendAsync(string token)
    {
        var result = await _httpClient.SendAsync(
            "https://RestfulSms/api/SMSLine",
            HttpMethod.Get,
    ...
    private async Task<bool> Send(string token, string message, string number, string lineToSend)
    {
        ...
        var result = await _httpClient.SendAsync(
            "https://RestfulSms/api/MessageSend",
            HttpMethod.Post,
            new Dictionary<string, string>
            {
                {"X-sms-ir-secure-token",token }
        ...
        var responseBody = await result.Content.ReadAsStringAsync();
        return JsonConvert.DeserializeObject<SendSmsResult>(responseBody).IsSuccessful;
    }
}
